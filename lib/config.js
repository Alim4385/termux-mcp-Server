...
// API reyestri: layihələr arası bir dəfə əlavə edilən API açarları (ad + məqsəd + key +
// confirm_before_use). Yeni alət YOXDUR — memory sistemi kimi mövcud view/create_file/
// str_replace alətləri ilə idarə olunur, "instructions" (lib/apis.js) vasitəsilə AI-ya
// izah edilir. Tək istifadəçi öz cihazında öz key-lərini idarə etdiyi üçün key AI-dan
// gizlədilmir (gizlətmək bu kontekstdə mənasızdır — AI onsuz da key-i işlətməlidir).
const APIS_DIR = path.join(WS, 'apis');
const APIS_FILE = path.join(APIS_DIR, 'keys.json');
...
if (!fs.existsSync(APIS_FILE)) fs.writeFileSync(APIS_FILE, '{}\n');
